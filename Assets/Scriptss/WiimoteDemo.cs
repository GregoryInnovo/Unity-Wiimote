...
public class WiimoteDemo : MonoBehaviour {
    ...
    */
    ...
    void Update () {
        ...
        {
            //UnityEngine.Debug.Log("Se oprime el boton de abajo");
            Camara.access.down = true;
        }
        else if (wiimote.Button.d_down != true)
        {
            Camara.access.down = false;
        }

        if (wiimote.Button.d_up)
        {
            //UnityEngine.Debug.Log("Se oprime el boton de arriba");
            Camara.access.up = true;
        }
        else if (wiimote.Button.d_up != true)
        {
            Camara.access.up = false;
        }
    ...
    void OnGUI()
    {
        ...
        if (wiimote != null && wiimote.current_ext != ExtensionController.NONE)
        {
            ...
            if (wiimote.current_ext == ExtensionController.NUNCHUCK) {
                ...
                if (data.stick[0] >= 119 && data.stick[0] <= 121 && data.stick[1] >= 138 && data.stick[1] <= 140)
                {
                    controlWii.transform.position = new Vector3(controlWii.transform.position.x, 0f, controlWii.transform.position.z);
                }
                if (data.stick[0] > 119 && data.stick[0] < 220 && data.stick[1] > 70 && data.stick[1] < 210)
                {
                    controlWii.transform.position = new Vector3(controlWii.transform.position.x + numUp, 0f, controlWii.transform.position.z);
                }
                if (data.stick[0] > 20 && data.stick[0] < 122 && data.stick[1] > 63 && data.stick[1] < 209)
                {
                    controlWii.transform.position = new Vector3(controlWii.transform.position.x - numUp, 0f, controlWii.transform.position.z);
                }
                if (data.stick[0] > 48 && data.stick[0] < 186 && data.stick[1] > 138 && data.stick[1] < 240)
                {
                    controlWii.transform.position = new Vector3(controlWii.transform.position.x , 0f, controlWii.transform.position.z + numUp);
                }
                if (data.stick[0] > 50 && data.stick[0] < 192 && data.stick[1] > 30 && data.stick[1] < 141)
                {
                    controlWii.transform.position = new Vector3(controlWii.transform.position.x, 0f, controlWii.transform.position.z - numUp);
                }
                if (data.z && butZ)
